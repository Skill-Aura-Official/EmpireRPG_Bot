...
from MythicRPG import DRAGONS
from MythicRPG.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def tsbssb_is_admin(chat_id: int):
    status = False
    tsbssb = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if tsbssb.id == user.id:
            status = True
            break
    return status
