...
from TSBSSB import TIGERS, WOLVES, dispatcher
...
from TSBSSB.modules.helper_funcs.extraction import (
    extract_text,
    extract_user,
    extract_user_and_text,
)
from TSBSSB.modules.helper_funcs.filters import CustomFilters
from TSBSSB.modules.helper_funcs.misc import split_message
from TSBSSB.modules.helper_funcs.string_handling import split_quotes
from TSBSSB.modules.log_channel import loggable
from TSBSSB.modules.sql import warns_sql as sql
from TSBSSB.modules.sql.approve_sql import is_approved
...
def warn(
    ...
    if is_user_admin(chat, user.id):
        # message.reply_text("Admins are protected by the TSB Council!")
        return

    if user.id in TIGERS:
        if warner:
            message.reply_text("Chief Policy Makers cannot be warned.")
        else:
            message.reply_text(
                "A Chief Policy Maker triggered an auto warn filter!\nI can't warn them but they should stay within guidelines.",
            )
        return

    if user.id in WOLVES:
        if warner:
            message.reply_text("TSB Managers are immune to warnings.")
        else:
            message.reply_text(
                "A TSB Manager triggered an auto warn filter!\nI can't warn them but they should stay within guidelines.",
            )
        return
    ...
    if num_warns >= limit:
        ...
        keyboard = InlineKeyboardMarkup(
            [[InlineKeyboardButton("🆘 Sᴜᴩᴩᴏʀᴛ", url="https://t.me/TSB_Council_Support")]]
        )
        log_reason = (
