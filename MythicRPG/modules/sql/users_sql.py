...
class Users(BASE):
    ...
    in_dm = Column(Boolean, default=False)

    def __init__(self, user_id, username=None, in_dm=False):
...
class ChatMembers(BASE):
    ...
    def __repr__(self):
        return "<Chat member {} in chat {}>".format(
            self.user,
            self.chat,
        )
...
# Dynamic schema update for existing databases
try:
    SESSION.execute("ALTER TABLE users ADD COLUMN in_dm BOOLEAN DEFAULT 0")
    SESSION.commit()
except Exception:
    SESSION.rollback()
...
def update_user(user_id, username, chat_id=None, chat_name=None, in_dm=None):
    with INSERTION_LOCK:
        user = SESSION.query(Users).get(user_id)
        if not user:
            user = Users(user_id, username, in_dm=bool(in_dm))
            SESSION.add(user)
            SESSION.flush()
        else:
            user.username = username
            if in_dm is not None:
                user.in_dm = bool(in_dm)
...
def num_users_in_chats():
    try:
        return SESSION.query(ChatMembers.user).distinct().count()
    finally:
        SESSION.close()
...
def num_users_dm():
    try:
        return SESSION.query(Users).filter(Users.in_dm == True).count()
    finally:
        SESSION.close()
...
def get_dm_users():
    try:
        return SESSION.query(Users).filter(Users.in_dm == True).all()
    finally:
        SESSION.close()
