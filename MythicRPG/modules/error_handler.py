...
from MythicRPG import DEV_USERS, OWNER_ID, dispatcher
...
def error_callback(update: Update, context: CallbackContext):
    ...
    pretty_message = (
        ...
        pretty_error,
        getattr(update.effective_user, 'id', 'None') if getattr(update, 'effective_user', None) else "None",
        getattr(update.effective_chat, 'title', '') if getattr(update, 'effective_chat', None) else "",
        getattr(update.effective_chat, 'id', '') if getattr(update, 'effective_chat', None) else "",
        getattr(update.callback_query, 'data', 'None') if getattr(update, 'callback_query', None) else "None",
        getattr(update.effective_message, 'text', 'No message') if getattr(update, 'effective_message', None) else "No message",
        tb,
    )
    try:
        key = requests.post(
    ...
    # ADDED LOGGING HERE TO SEE THE ACTUAL ERROR!
    import logging
    logging.getLogger(__name__).error(pretty_message)
