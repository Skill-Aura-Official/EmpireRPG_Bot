...
REQUIRED_CHATS = [
    ("@TSB_Council_Support", "Support group", "https://t.me/TSB_Council_Support"),
    (-1002151810999, "1st chatting gc", "https://t.me/+MIs9myZi-tw1MGI1"),
    (-1003882113270, "2nd chatting gc", "https://t.me/+IH6lS7uIFDBmNGM1"),
    ("@TSB_Bots", "tsb bot update channel", "https://t.me/TSB_Bots"),
    ("@TSB_Council", "tsb main channel", "https://t.me/TSB_Council")
]

def check_user_membership(bot, user_id):
    """
    Checks if a user is a member of all required chats.
    Returns (is_member, list_of_missing_chats_with_links)
    """
    if user_id == OWNER_ID or user_id in DEV_USERS:
        return True, []

    missing = []
    for chat_target, chat_name, invite_link in REQUIRED_CHATS:
        try:
            member = bot.get_chat_member(chat_target, user_id)
            if member.status in ["left", "kicked"]:
                missing.append((chat_name, invite_link))
        except Exception:
            missing.append((chat_name, invite_link))
...
def is_user_admin(chat: Chat, user_id: int, member: ChatMember = None) -> bool:
    ...
    ):  # Count telegram and Group TEXT as admin
        return True
    if not member:
        with THREAD_LOCK:
