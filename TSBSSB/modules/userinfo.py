...
import TSBSSB.modules.sql.userinfo_sql as sql
from TSBSSB import (
    DEMONS,
    DEV_USERS,
    DRAGONS,
...
from TSBSSB.__main__ import STATS, TOKEN, USER_INFO
from TSBSSB.modules.disable import DisableAbleCommandHandler
from TSBSSB.modules.helper_funcs.chat_status import sudo_plus
...
from TSBSSB.modules.sql.users_sql import get_user_num_chats
...
def info(update: Update, context: CallbackContext):
    ...
    if user.id == OWNER_ID:
        text += "\n\n🔱 <b>TSB FOUNDER</b> — Highest Authority.\n"
    elif user.id in DEV_USERS:
        text += "\n\n🔱 This user is a <b>TSB CO-FOUNDER</b>.\n"
    elif user.id in DRAGONS:
        text += "\n\n🔱 This user is a <b>TSB GENERAL SECRETARY</b>.\n"
    elif user.id in DEMONS:
        text += "\n\n⚜️ This user is a <b>TSB SENIOR ADVISOR</b>.\n"
    elif user.id in TIGERS:
        text += "\n\n⚜️ This user is a <b>TSB CHIEF POLICY MAKER</b>.\n"
    elif user.id in WOLVES:
        text += "\n\n⚜️ This user is a <b>TSB MANAGER</b>.\n"
            ...
            message.reply_photo(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "TSB Support", url=f"https://t.me/{SUPPORT_CHAT}"
                            ),
                        ],
                    ]
                ),
...
def set_about_bio(update: Update, context: CallbackContext):
    ...
    if message.reply_to_message:
        ...
        if user_id in [777000, 1087968824] and sender_id not in DEV_USERS:
            message.reply_text("» You are not authorized to edit TSB Council metadata.")
            return

        if user_id == bot.id and sender_id not in DEV_USERS:
            message.reply_text(
                "» I only trust the TSB Council to manage my identity."
            )
            return
