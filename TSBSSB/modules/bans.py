...
from TSBSSB.modules.helper_funcs.extraction import extract_user_and_text
from TSBSSB.modules.helper_funcs.string_handling import extract_time
from TSBSSB.modules.log_channel import gloggable, loggable
...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    if is_user_ban_protected(chat, user_id, member) and user not in DEV_USERS:
        if user_id == OWNER_ID:
            message.reply_text("Trying to put me against the Founder of the TSB Council?")
        elif user_id in DEV_USERS:
            message.reply_text("I can't act against our own.")
        elif user_id in DRAGONS:
            message.reply_text(
                "A General Secretary is immune to standard group bans."
            )
        elif user_id in DEMONS:
            message.reply_text(
                "I require a direct order from the Council to act against a Senior Advisor."
            )
        elif user_id in TIGERS:
            message.reply_text(
                "Chief Policy Makers are protected by the TSB Council."
            )
        elif user_id in WOLVES:
            message.reply_text("TSB Managers are immune to standard ban commands.")
        else:
    ...
    try:
        ...
        bot.sendMessage(chat.id, reply, parse_mode=ParseMode.HTML, reply_markup=keyboard)
        return log
